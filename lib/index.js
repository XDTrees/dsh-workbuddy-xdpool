...
async function authFilesIn(dir) {
	...
	const files = entries.filter((name) => {
		if (name === "workbuddy-desktop.info") return true;
...
*/
async function poolWebStatus(deps) {
...
const Config = z.object({
	...
	cooldownMs: z.number().step(1).min(1e3).description("Rate-limit cooldown per account, in milliseconds")
});
...
function apply(ctx, config = {}) {
	...
	ctx.inject(["settings"], (settingsCtx) => {
		...
		if (typeof service.installSection === "function") service.installSection(ctx, WORKBUDDY_POOL_SETTINGS_NS, Config, config, sectionHooks);
	...
	shim.ready.then(async () => {
		...
		(async () => {
			try {
				const credential = (await core.pool.scan())[0]?.credential;
...
//#endregion
export { Config, FALLBACK_WORKBUDDY_MODELS, POOL_CHECKIN_PATH, POOL_RESCAN_PATH, POOL_RESET_COOLDOWN_PATH, POOL_STATUS_PATH, WORKBUDDY_AUTH_FILE_ENV, WORKBUDDY_LIVE_FILENAME, WORKBUDDY_POOL_PROVIDER, WORKBUDDY_POOL_SETTINGS_NS, WorkBuddyAccountPool, WorkBuddyCatalog, WorkBuddyUpstreamClient, apply, buildStatus, candidateAuthDirs, classifyUpstreamError, createCore, createWorkBuddyAdapter, createWorkBuddyShim, currentApi, defaultDesktopAuthDirs, formatRates, formatStatus, inject, name, parseRateLimitReset, parseWorkBuddyAuth, setApi, workbuddyAccountId };
