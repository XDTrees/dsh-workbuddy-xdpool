...
function createWorkBuddyAdapter(options) {
	...
	return {
		...
		adapter: new PiAiAdapter({
			...
			resolveImageAccess: (attachments, ref) => ctx ? resolveImageAttachmentAccess(attachments, (hostPath) => ctx.get("fs")?.processPathFromHostPath(hostPath) ?? hostPath, ref) : void 0
		}),
		buildModels,
		invalidate: () => {
