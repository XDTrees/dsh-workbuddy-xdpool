	...
	*/
	visible() {
		...
		return this.models.filter((model) => allow === void 0 || allow.has(model.id)).map((model) => {
			const next = { ...model };
			if (imageSet !== void 0) next.supportsImages = imageSet.has(model.id);
...
async function poolWebStatus(deps, region = "cn") {
	...
	const selection = deps.catalog.currentSelection();
	const rows = [];
	const now = Date.now();
	for (const account of accounts) {
...
function apply(ctx, config = {}) {
	...
	ctx.inject(["settings"], (settingsCtx) => {
		...
		settingsService = typeof service.set === "function" ? service : void 0;
	});
	/**
